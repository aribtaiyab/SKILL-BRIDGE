...
export default function OpportunityDetailsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/opportunities/${opportunityId}`).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`/api/student/opportunities/${opportunityId}/readiness`).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`/api/applications`).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`/api/student/opportunities/saved`).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`/api/opportunities/${opportunityId}/proof`).then(r => r.ok ? r.json() : null).catch(() => null),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/opportunities/${opportunityId}/save`, { method })

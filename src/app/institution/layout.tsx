...
export default function InstitutionLayout({ children }: { children: React.ReactNode }) {
  ...
  const displayName = profile?.full_name || user?.email?.split('@')[0] || 'Institution'
  const initials = (profile?.full_name || 'IN').split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()
        ...
        <header className="h-16 flex items-center justify-between px-6 border-b border-[var(--color-border-primary)] bg-[var(--color-surface-card)]">
          ...
          <div className="ml-auto flex items-center gap-3">
            <div className="flex items-center gap-2 text-sm font-medium">
              <div className="h-8 w-8 bg-[var(--color-surface-secondary)] border border-[var(--color-border-primary)] rounded-full flex items-center justify-center">
                <Building2 className="h-4 w-4 text-[var(--color-text-secondary)]" />
              </div>

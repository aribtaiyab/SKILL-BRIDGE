...
export const LEVEL_1_KNOWLEDGE_ASSESSMENTS: Level1Assessment[] = [
  ...
  {
    id: "assess-l1-mongodb-core",
    title: "MongoDB Aggregations & Document Modeling Benchmark",
    skill: "MongoDB",
    description: "Evaluates document modeling, aggregation pipelines, indexing, and BSON fundamentals in MongoDB.",
    timeLimitMinutes: 10,
    totalQuestions: 3,
    passingScore: 70,
    difficulty: "Intermediate",
    questions: [
      {
        id: "q-mongo-1",
        questionText: "What data format does MongoDB utilize natively to store and transmit documents with typed extensions?",
        points: 34,
        orderIndex: 1,
        options: [
          { id: "opt-m1a", optionText: "Binary JSON (BSON)" },
          { id: "opt-m1b", optionText: "Raw XML text" },
          { id: "opt-m1c", optionText: "YAML serialized streams" },
          { id: "opt-m1d", optionText: "Protocol Buffers" },
        ],
        correctOptionId: "opt-m1a",
        explanation: "MongoDB stores documents in BSON format, which provides binary serialization with support for additional data types like dates and binary data.",
      },
      {
        id: "q-mongo-2",
        questionText: "Which aggregation stage is used to filter documents in a MongoDB aggregation pipeline similar to a query predicate?",
        points: 33,
        orderIndex: 2,
        options: [
          { id: "opt-m2a", optionText: "$match" },
          { id: "opt-m2b", optionText: "$filter" },
          { id: "opt-m2c", optionText: "$where" },
          { id: "opt-m2d", optionText: "$group" },
        ],
        correctOptionId: "opt-m2a",
        explanation: "The $match stage filters the document stream to allow only matching documents to pass through to the next pipeline stage.",
      },
      {
        id: "q-mongo-3",
        questionText: "In a compound index on { status: 1, createdAt: -1 }, which query CANNOT take full advantage of the index prefix?",
        points: 33,
        orderIndex: 3,
        options: [
          { id: "opt-m3a", optionText: "Queries filtering only on { createdAt: { $gt: date } }" },
          { id: "opt-m3b", optionText: "Queries filtering on { status: 'active' }" },
          { id: "opt-m3c", optionText: "Queries filtering on { status: 'active', createdAt: { $gt: date } }" },
          { id: "opt-m3d", optionText: "Queries sorting on { status: 1, createdAt: -1 }" },
        ],
        correctOptionId: "opt-m3a",
        explanation: "MongoDB compound indexes can only be used from left to right (index prefix). Without the 'status' leading key, 'createdAt' queries require a collection scan or secondary index.",
      },
    ],
  },
  {
    id: "assess-l1-express-core",
    title: "Express.js Middleware Architecture & Routing Benchmark",
    skill: "Express.js",
    description: "Tests request lifecycle, middleware chaining, routing, and centralized error handling in Express.",
    timeLimitMinutes: 10,
    totalQuestions: 3,
    passingScore: 70,
    difficulty: "Intermediate",
    questions: [
      {
        id: "q-exp-1",
        questionText: "In Express middleware functions, what function must be called to pass control to the subsequent middleware?",
        points: 34,
        orderIndex: 1,
        options: [
          { id: "opt-e1a", optionText: "next()" },
          { id: "opt-e1b", optionText: "yield()" },
          { id: "opt-e1c", optionText: "continue()" },
          { id: "opt-e1d", optionText: "forward()" },
        ],
        correctOptionId: "opt-e1a",
        explanation: "Invoking next() transfers execution to the next middleware in the stack.",
      },
      {
        id: "q-exp-2",
        questionText: "How does Express distinguish an error-handling middleware from a standard request middleware?",
        points: 33,
        orderIndex: 2,
        options: [
          { id: "opt-e2a", optionText: "It explicitly accepts 4 parameters: (err, req, res, next)" },
          { id: "opt-e2b", optionText: "It must be named app.useError()" },
          { id: "opt-e2c", optionText: "It must return an Error instance synchronously" },
          { id: "opt-e2d", optionText: "It is declared using async/await exclusively" },
        ],
        correctOptionId: "opt-e2a",
        explanation: "Express checks function.length === 4 to identify error-handling middleware.",
      },
      {
        id: "q-exp-3",
        questionText: "For route path '/api/users/:userId', how do you access the dynamic URL segment in Express?",
        points: 33,
        orderIndex: 3,
        options: [
          { id: "opt-e3a", optionText: "req.params.userId" },
          { id: "opt-e3b", optionText: "req.query.userId" },
          { id: "opt-e3c", optionText: "req.body.userId" },
          { id: "opt-e3d", optionText: "req.segments.userId" },
        ],
        correctOptionId: "opt-e3a",
        explanation: "Named route parameters are populated on req.params.",
      },
    ],
  },
  {
    id: "assess-l1-auth-security",
    title: "Authentication, JWT & Web Security Benchmark",
    skill: "Authentication",
    description: "Evaluates JSON Web Tokens, password hashing, session management, CORS, and XSS/CSRF mitigation.",
    timeLimitMinutes: 10,
    totalQuestions: 3,
    passingScore: 70,
    difficulty: "Intermediate",
    questions: [
      {
        id: "q-sec-1",
        questionText: "Why should sensitive JWT tokens be stored in HttpOnly cookies rather than localStorage in browser clients?",
        points: 34,
        orderIndex: 1,
        options: [
          { id: "opt-s1a", optionText: "HttpOnly cookies cannot be read or stolen by malicious client-side JavaScript via XSS attacks" },
          { id: "opt-s1b", optionText: "LocalStorage cannot store strings longer than 256 characters" },
          { id: "opt-s1c", optionText: "Cookies are encrypted automatically by the OS kernel" },
          { id: "opt-s1d", optionText: "HttpOnly cookies bypass CORS verification entirely" },
        ],
        correctOptionId: "opt-s1a",
        explanation: "HttpOnly cookies are inaccessible to JavaScript Document.cookie, neutralizing token theft via Cross-Site Scripting (XSS).",
      },
      {
        id: "q-sec-2",
        questionText: "What is the primary role of a salt when hashing passwords with bcrypt or argon2?",
        points: 33,
        orderIndex: 2,
        options: [
          { id: "opt-s2a", optionText: "To guarantee unique hashes for identical passwords and defend against precomputed rainbow table attacks" },
          { id: "opt-s2b", optionText: "To compress the password so it fits into fixed DB column lengths" },
          { id: "opt-s2c", optionText: "To decrypt passwords when an admin triggers password recovery" },
          { id: "opt-s2d", optionText: "To sign the JWT payload with symmetric keys" },
        ],
        correctOptionId: "opt-s2a",
        explanation: "Salting appends unique random data to each password before hashing, rendering rainbow tables ineffective.",
      },
      {
        id: "q-sec-3",
        questionText: "Which HTTP header defends against Clickjacking attacks by preventing your application from being embedded in an <iframe>?",
        points: 33,
        orderIndex: 3,
        options: [
          { id: "opt-s3a", optionText: "X-Frame-Options: DENY (or Content-Security-Policy frame-ancestors 'none')" },
          { id: "opt-s3b", optionText: "Access-Control-Allow-Origin: *" },
          { id: "opt-s3c", optionText: "X-XSS-Protection: 1" },
          { id: "opt-s3d", optionText: "Strict-Transport-Security" },
        ],
        correctOptionId: "opt-s3a",
        explanation: "X-Frame-Options prevents the page from being rendered in an iframe, stopping clickjacking attacks.",
      },
    ],
  },
  {
    id: "assess-l1-deployment-cloud",
    title: "Containerization, Cloud Deployment & CI/CD Benchmark",
    skill: "Deployment",
    description: "Tests Docker containerization, CI/CD automated pipelines, cloud hosting, and process management.",
    timeLimitMinutes: 10,
    totalQuestions: 3,
    passingScore: 70,
    difficulty: "Intermediate",
    questions: [
      {
        id: "q-dep-1",
        questionText: "What is the primary operational advantage of Docker containerization over traditional Virtual Machines?",
        points: 34,
        orderIndex: 1,
        options: [
          { id: "opt-d1a", optionText: "Containers share the host OS kernel and isolate at the process level, resulting in faster startup and lower resource footprint" },
          { id: "opt-d1b", optionText: "Containers run without needing any CPU cycles" },
          { id: "opt-d1c", optionText: "Containers do not require networking configuration" },
          { id: "opt-d1d", optionText: "Containers eliminate the need for application code" },
        ],
        correctOptionId: "opt-d1a",
        explanation: "Containers share the host OS kernel, making them lightweight, rapid to start, and highly resource-efficient compared to hypervisor VMs.",
      },
      {
        id: "q-dep-2",
        questionText: "Where should production database credentials and API secrets be supplied in a modern containerized deployment?",
        points: 33,
        orderIndex: 2,
        options: [
          { id: "opt-d2a", optionText: "Injected securely via environment variables or secret managers (e.g. AWS Secrets Manager, Vault) at runtime" },
          { id: "opt-d2b", optionText: "Hardcoded directly into the Dockerfile source image" },
          { id: "opt-d2c", optionText: "Committed into public Git repository READMEs" },
          { id: "opt-d2d", optionText: "Encoded into client-side bundle JS files" },
        ],
        correctOptionId: "opt-d2a",
        explanation: "12-Factor App methodology mandates strict separation of config/secrets from code via environment variables and runtime secrets management.",
      },
      {
        id: "q-dep-3",
        questionText: "In a Continuous Integration (CI) pipeline, which phase must succeed before automated production deployment is triggered?",
        points: 33,
        orderIndex: 3,
        options: [
          { id: "opt-d3a", optionText: "Automated linting, typechecking, unit tests, and build verification" },
          { id: "opt-d3b", optionText: "Manual code printing and physical document filing" },
          { id: "opt-d3c", optionText: "Restarting all developer laptop machines" },
          { id: "opt-d3d", optionText: "Flushing the DNS cache of the local router" },
        ],
        correctOptionId: "opt-d3a",
        explanation: "CI validates code health through automated linting, test suites, and build steps prior to triggering CD deployment.",
      },
    ],
  },
  {
    id: "assess-l1-dsa-core",
    title: "Data Structures & Algorithmic Problem Solving Benchmark",
    skill: "Problem Solving / DSA",
    description: "Evaluates time/space complexity analysis, lookup structures, sorting, and algorithmic optimization.",
    timeLimitMinutes: 10,
    totalQuestions: 3,
    passingScore: 70,
    difficulty: "Intermediate",
    questions: [
      {
        ...
        orderIndex: 1,
        options: [
          { id: "opt-ds1a", optionText: "O(log N)" },
          { id: "opt-ds1b", optionText: "O(N)" },
          { id: "opt-ds1c", optionText: "O(N log N)" },
          { id: "opt-ds1d", optionText: "O(1)" },
        ],
        correctOptionId: "opt-ds1a",
        explanation: "Binary search halves the search space at each iteration, yielding logarithmic O(log N) time complexity.",
      },
      {
        id: "q-dsa-2",
        questionText: "What is the average time complexity for insertion, deletion, and search operations in a Hash Table with a good hash function?",
        points: 33,
        orderIndex: 2,
        options: [
          { id: "opt-ds2a", optionText: "O(1) constant time" },
          { id: "opt-ds2b", optionText: "O(N) linear time" },
          { id: "opt-ds2c", optionText: "O(log N)" },
          { id: "opt-ds2d", optionText: "O(N^2)" },
        ],
        correctOptionId: "opt-ds2a",
        explanation: "Hash tables provide average O(1) constant time complexity for basic operations when collisions are minimized.",
      },
      {
        id: "q-dsa-3",
        questionText: "Which abstract data type follows the Last-In, First-Out (LIFO) order of elements?",
        points: 33,
        orderIndex: 3,
        options: [
          { id: "opt-ds3a", optionText: "Stack" },
          { id: "opt-ds3b", optionText: "Queue" },
          { id: "opt-ds3c", optionText: "Linked List" },
          { id: "opt-ds3d", optionText: "Binary Search Tree" },
        ],
        correctOptionId: "opt-ds3a",
        explanation: "A stack is a LIFO data structure where the most recently added item is the first one removed.",
      },
    ],
  },
  {
    id: "assess-l1-html-basics",
    title: "Semantic HTML5 & Web Standards Benchmark",
    skill: "HTML",
    description: "Evaluates modern semantic HTML elements, accessible forms, meta tags, and document outline.",
    timeLimitMinutes: 10,
    totalQuestions: 3,
    passingScore: 70,
    difficulty: "Beginner",
    questions: [
      {
        id: "q-h-1",
        questionText: "Which semantic HTML5 element should be used for the primary navigation links of a website?",
        points: 34,
        orderIndex: 1,
        options: [
          { id: "opt-h1a", optionText: "<nav>" },
          { id: "opt-h1b", optionText: "<section>" },
          { id: "opt-h1c", optionText: "<aside>" },
          { id: "opt-h1d", optionText: "<div>" },
        ],
        correctOptionId: "opt-h1a",
        explanation: "The <nav> element represents a section of a page whose purpose is to provide navigation links.",
      },
      {
        id: "q-h-2",
        questionText: "Why is the 'alt' attribute required on <img> elements for production web accessibility?",
        points: 33,
        orderIndex: 2,
        options: [
          { id: "opt-h2a", optionText: "It supplies assistive technologies (screen readers) with text alternatives and renders if the image fails to load" },
          { id: "opt-h2b", optionText: "It sets the visual width of the image in pixels" },
          { id: "opt-h2c", optionText: "It compresses the image filesize in transit" },
          { id: "opt-h2d", optionText: "It applies CSS filters to the image" },
        ],
        correctOptionId: "opt-h2a",
        explanation: "The alt attribute provides a text alternative for screen readers and search engines, and shows fallback text when images fail to render.",
      },
      {
        id: "q-h-3",
        questionText: "What is the purpose of '<!DOCTYPE html>' at the very top of an HTML document?",
        points: 33,
        orderIndex: 3,
        options: [
          { id: "opt-h3a", optionText: "Informs the browser to render the page in standard modern HTML5 mode instead of quirks mode" },
          { id: "opt-h3b", optionText: "Imports JavaScript libraries from the CDN" },
          { id: "opt-h3c", optionText: "Defines the server IP address" },
          { id: "opt-h3d", optionText: "Encrypts HTML source code from viewers" },
        ],
        correctOptionId: "opt-h3a",
        explanation: "The DOCTYPE declaration ensures the browser renders the page using modern standard compliance mode rather than legacy quirks mode.",
      },
    ],
  },
  {
    id: "assess-l1-css-layouts",
    title: "Modern CSS, Flexbox & Responsive Layouts Benchmark",
    skill: "CSS",
    description: "Evaluates CSS box model, Flexbox alignments, responsive media queries, and layout architecture.",
    timeLimitMinutes: 10,
    totalQuestions: 3,
    passingScore: 70,
    difficulty: "Beginner",
    questions: [
      {
        id: "q-c-1",
        questionText: "In CSS Flexbox, which property aligns items along the cross axis (perpendicular to flex-direction)?",
        points: 34,
        orderIndex: 1,
        options: [
          { id: "opt-c1a", optionText: "align-items" },
          { id: "opt-c1b", optionText: "justify-content" },
          { id: "opt-c1c", optionText: "flex-wrap" },
          { id: "opt-c1d", optionText: "order" },
        ],
        correctOptionId: "opt-c1a",
        explanation: "align-items controls alignment on the cross axis, while justify-content controls alignment on the main axis.",
      },
      {
        id: "q-c-2",
        questionText: "What effect does setting 'box-sizing: border-box' have on an element?",
        points: 33,
        orderIndex: 2,
        options: [
          { id: "opt-c2a", optionText: "Padding and border are included in the element's total specified width and height" },
          { id: "opt-c2b", optionText: "Padding is added outside the element's width, expanding total footprint" },
          { id: "opt-c2c", optionText: "Borders become semi-transparent" },
          { id: "opt-c2d", optionText: "The element converts into a flexible grid container" },
        ],
        correctOptionId: "opt-c2a",
        explanation: "border-box ensures padding and borders are included in the element's declared width and height, simplifying sizing calculations.",
      },
      {
        id: "q-c-3",
        questionText: "Which CSS feature allows styling rules to adapt conditionally based on viewport width or device characteristics?",
        points: 33,
        orderIndex: 3,
        options: [
          { id: "opt-c3a", optionText: "Media queries (@media)" },
          { id: "opt-c3b", optionText: "CSS Keyframes (@keyframes)" },
          { id: "opt-c3c", optionText: "CSS Variables (:root)" },
          { id: "opt-c3d", optionText: "Font faces (@font-face)" },
        ],
        correctOptionId: "opt-c3a",
        explanation: "@media queries allow styles to adapt based on viewport size, orientation, and device display features.",
      },
    ],
  },
]

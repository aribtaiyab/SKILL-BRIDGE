...
export default function AcademiaLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-[#F8FAFC]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-bold text-slate-500">Verifying authorized Academia credentials...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden text-slate-900">
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-64 flex-col border-r border-slate-200/80 bg-white shadow-xs">
        {/* Brand Header */}
        <div className="h-16 flex items-center px-6 border-b border-slate-100 justify-between">
          <Link href="/academia" className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-600 text-white font-black text-xs shadow-xs">
              SC
            </div>
            <div>
              <div className="font-black text-sm tracking-tight text-slate-900">SkillBridge</div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/70">
          ...
          <Link
            href="/"
            className="flex items-center gap-2 w-full px-3 py-1.5 rounded-lg text-xs font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-colors mb-1"
          >
          ...
          <Link
            href="/select-role"
            className="flex items-center gap-2 w-full px-3 py-1.5 rounded-lg text-xs font-bold text-emerald-700 hover:bg-emerald-50 transition-colors mb-1"
          >
          ...
          <Button
            ...
            size="sm"
            onClick={signOut}
            className="w-full justify-start text-xs font-bold text-slate-500 hover:text-rose-600 hover:bg-rose-50 h-8 rounded-lg"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-16 flex items-center justify-between px-4 sm:px-8 border-b border-slate-200/80 bg-white/90 backdrop-blur-md shadow-xs z-30">
          <div className="flex items-center gap-3">
            <button
          ...
          <div className="lg:hidden fixed inset-0 z-40 bg-slate-900/40 backdrop-blur-xs flex">
            <div className="w-72 bg-white h-full shadow-2xl flex flex-col p-4 space-y-4">
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold ${
                        isActive ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      <div className="flex items-center gap-3">
              ...
              <div className="pt-2 border-t space-y-1">
                ...
                <Link
                  href="/select-role"
                  className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold text-emerald-700 hover:bg-emerald-50"
                >
                  <UserCheck className="h-3.5 w-3.5 text-emerald-600" />
                  <span>Switch Role</span>
                </Link>

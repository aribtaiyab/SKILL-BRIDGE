...
export async function GET(request: NextRequest) {
  try {
    ...
    if (requirements.length === 0) {
      const benchmark = CAREER_BENCHMARK_PROFILES.find(c => c.id === targetCareerId || c.slug === targetCareerId) || CAREER_BENCHMARK_PROFILES[0]
    ...
    const readiness = evaluateCareerReadiness(targetCareerName, requirements, studentSkills)
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        gaps: readiness.skills,
        criticalGaps: readiness.criticalGaps,
        nearReadySkills: readiness.nearReadySkills,
        strengths: readiness.strengths,
        priorityGap: readiness.priorityGap,
    ...
    return NextResponse.json({
      success: false,
      error: err.message || 'Failed to fetch skill gaps',
    }, { status: 500 })
  }
}
